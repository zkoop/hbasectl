  ...
  module Tools
    ...
    class Splitter < Base
      ...
      def initialize(opts = {} )
        ...
        @work_queue_percent = opts.delete(:percent) || 5 # percentage of regions to split in one run. Default: 5. No more than 5% of regions should not be splitted in one shot.
        ...
        # znode path for split tasks
        @split_base_path = File.join(@znode_base,"split") # /admin/hbase/cluster/split

        # base znode path for locks
        @lock_base_path = File.join(@split_base_path,"lock",File.basename(@input).gsub(/\..*/,'' )) # /admin/hbase/cluster/split/lock/split_1234567891

        # lock znode name
        @lock_node=File.join(@lock_base_path,"lock") # /admin/hbase/cluster/split/lock/split_1234567891/lock

        # znode path for keeping state of this run
        @inprogress_path = File.join(@split_base_path,"inprogress",File.basename(@input).gsub(/\..*/,'' ))
        ...
        # create /admin/hbase/cluster-hb1/split
        unless HbaseCtl.dryrun
          unless @admin.znode_exists?(@split_base_path)
            @admin.create_znode(@split_base_path, { :mode => 0}) 
            HbaseCtl.log("#{HbaseCtl.hostname}:#@command:# Created znode #@split_base_path")
          end
        else
          HbaseCtl.log("#{HbaseCtl.hostname}:#@command:# Created znode #@split_base_path")
        end

        # create lock base /admin/hbase/cluster-hb1/split/lock/split_1234567891
        unless HbaseCtl.dryrun
      ...
      def run!
        ...
        if verify_lock(my_lock)
          # consumer thread that does the split
		  ...
		  # check whether I got a hash or  raise exception
		      ...
		      HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# consumer thread got work for #{table_name}")
                      split_queue = Array.new
                      num_regions_in_split = 0 
                      #if there are any znodes then there might be split running for that table. 
                      
                      # create a node for table /admin/hbase/cluster/split/split_UNIXTIME/inprogress/table
			  ...
			  # create a persistent node
                          ...
                          #since the table_znode exist verify the chidren/region nodes and verify if they are doing split
                          #This is needed to stay away from starting too many split requests
                          begin
                            regions_in_znode = @admin.get_children(table_znode_path,false)

                            regions_in_znode.each do |region_name|
                              # get_split_state return an array of hashes
                              split_state = @admin.get_split_state(region_name)

                              # get the first value , and key of the hash
			      case split_state.first[region_name]
                              when HbaseCtl::Constants::RS_REGION_SPLITTING
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} currently splitting, skipping this region")
			        num_regions_in_split += 1 # currently this is not used but might need in future to throttle regions across split instances
                                ...
                                #HbaseCtl.log("#{current_thread}#@command:#{current_thread}:# table: #{table_name} region: #{region_name} not splitting, adding to the queue")
                                #split_queue << region_name
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} not splitting, removing the znode")
                                # if regin needs split next input will have this region. Revisit this to do request split anyway 
                        ...
                        regions.each do |region_name|
                          begin
                            ...
                            if @admin.znode_exists?(region_path) 
                              HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# #{region_path} exists .. checking the split state ")
     			      split_state = @admin.get_split_state(region_name)

                              case split_state.first[region_name]
                              when HbaseCtl::Constants::RS_REGION_SPLITTING
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} split state: in progress. Not initiating split")
                                mutex.synchronize do
                                  @progress << { "table" => table_name, "region" => region_name , "task" => @command , "status" => "skipped", "time" => Time.now.to_s}
                                end
                              when HbaseCtl::Constants::RS_REGION_SPLIT_NONE
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} split state: not started")
                                split_queue << region_name
                              when HbaseCtl::Constants::RS_REGION_SPLIT
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} state: split completed ")
                                ...
                                mutex.synchronize do
                                  @progress << { "table" => table_name, "region" => region_name , "task" => @command , "status" => "done", "time" => Time.now.to_s}
                                end
                              end #case ends
			      ...
			      region_znode = @admin.create_znode(region_path,{:mode => 0 , :info => "#{Time.now.to_i}"}) # put a meaningful information ?
                              ...
                              HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name}  Initiating split")
                              split_queue << region_name
                            end #if ends
		        ...
		        # number of regions to split in one pass.
                        num_region_threshold = (@work_queue_percent.fdiv(100) * split_queue.length).ceil
                        num_region_threshold = split_queue.length if num_region_threshold > split_queue.length
                        limit = num_region_threshold

                        if split_queue.length <= 10 # if the number of regions < 10 do  for all 
                          limit = split_queue.length 
                        end
                        HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name}  region_concurrency: #{limit} split_queue_length: #{split_queue.length} ")
                  
                        split_work_queue = Array.new
                        done_queue = Array.new
                        inprogress_queue = Array.new
                        loop_count = 0

                        # this is a hack to break out of the loop if the status of split is NONE for a long time
                        # There is no easy way to check the status of a region split if it fails
                        # this can be used to assume the split failed

                        split_loop_count = {}

                        # iterate over each region that we selected and do split
                        # pause if we reached the limit , till we have any free slot
                        split_queue.each do |region_name|
                          loop_count +=1

                          split_loop_count[region_name] = 0

                          split_work_queue << region_name
                          inprogress_queue << region_name
                          HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region_concurrency: #{limit} split_work_queue: #{split_work_queue.length} inprogress_queue: #{inprogress_queue.length}  split_queue_length: #{split_queue.length} ")
                          region_path = File.join(table_znode_path,region_name)
                          begin 
                            @admin.split(region_name)
                          rescue java.io.IOException, java.lang.InterruptedException => e
                            HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# #{e.message} #{e.inspect} #{e.backtrace}")
                            HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region_name} failed split. ")
                            mutex.synchronize do 
                            ...
                            split_work_queue.delete(region_name)
                            loop_count -=1
                          end #begin end
 
                          if loop_count == limit
                            loop do
                            # sleep 30 seconds and check the status of split
                              HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# sleeping for 30 seconds till split completes")
                              sleep 30
                        
                              # returns the status of each region as an array of hashes 
                              split_status = @admin.get_split_state(inprogress_queue)

                              split_status.each do |region_status| # region will be a hash with region name as key
                                region,status = region_status.shift
                                region_path = File.join(table_znode_path,region)

                                if status ===  HbaseCtl::Constants::RS_REGION_SPLIT # RS split has completed
                                  ...
                                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} split: completed status: #{status}")
		                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} removing znode #{region_path}")
		                    ...
		                    HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} removed znode #{region_path}")
                                ...
                                elsif status === HbaseCtl::Constants::RS_REGION_SPLITTING
                                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} split:  still in progress status: #{status}") 
                                  begin
                                ...
                                elsif status === HbaseCtl::Constants::RS_REGION_SPLIT_NONE

                                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} split:  NONE status: #{status}")

                                  if split_loop_count[region] == 10
                                    # assume the split has failed even after waiting for 30*10 seconds.
                                    # As of now this will be a fair assumption to break out of the loop if the status is NONE for a long time.
                                    # This means the region is still available in .META and there are no daughter regions created
                                    # there is no easy way to check the status of a region split, the results are not exposed via the api's
                                    HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} region has not started splitting after 5 tries, forcefully failing the region")
                                    done_queue << region
                                    inprogress_queue.delete(region) || HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} not found in inprogress_queue")
                                    loop_count -= 1
                                    ...
                                    # update the progress to the master thread
                                    mutex.synchronize do
                                      @progress << { "table" => table_name, "region" => region, "task" => @command , "status" => "failed", "time" => Time.now.to_s}
                                    end

                                    split_loop_count[region] = 0
                                    ...
                                    split_loop_count[region] +=1
                                  end   # inner if end

                                else
                                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} got status status: #{status}")
                                  HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} removing znode #{region_path}")

                                  # got the rs eventtype that we are not expecting, delete the znode
                                  begin
                                    @admin.delete_znode(region_path,-1)
                                    HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} region: #{region} removed znode #{region_path}")
                                  rescue org.apache.zookeeper.KeeperException, java.lang.InterruptedException, java.lang.IllegalArgumentException => e
                                  ...
                                  # update the progress to the master thread
                                  mutex.synchronize do 
                                    @progress << { "table" => table_name, "region" => region, "task" => @command , "status" => "#{status}", "time" => Time.now.to_s}
                                  end
                                end #if ends 
                              end # block ends

                              if done_queue.length == split_queue.length 
				                        # We are done with this table. Now delete the table znode and break out of the loop and finishes thread
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} Finished #{done_queue.length} out of #{split_queue.length}")
                              ...
                              elsif split_work_queue.length < split_queue.length && loop_count < limit
                                break

                              elsif split_work_queue.length == split_queue.length && done_queue.length < split_queue.length
                                HbaseCtl.log("#{HbaseCtl.hostname}:#@command:#{current_thread}:# table: #{table_name} waiting for all regions to finish split. Finished #{done_queue.length} out of #{split_queue.length}")
                              end #if ends

                            end # loop ends
                          end # if loop_count ends
		        end #split queue block ends
        ...
        # remove  @inprogress_path /admin/hbase/cluster/hbasectl/split/inprogress/split_1382576717/
